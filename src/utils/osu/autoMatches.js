...
const { startingElo, duelElo, addElo, getElo, updateRecord, peakElo, getPlacementElo } = require(`./skillsCalculation`);
...
async function getApiClient() {
    if (Date.now() < tokenExpiresAt - 60000) { // refresh 1 min early
        return new Client(cachedToken.access_token);
    }
    ...
    async handleLobby(osuUser1, osuUser2, interaction, client, teams, tournament, selectedPool, customBO, customELO) {
        try {
            ...
            /* 
                ...
                If teams is empty then make the match 1v1
            */
            ...
            // Legacy v1 client (faster + higher limits for simple user lookups)
            const legacyUserCache = new Map(); 

            for (let i = 0; i < playerArray.length; i++) {
                let dbName = playerArray[i];
                let playerProfile = await osuUser.findOne({ osuUserName: dbName });

                const userId = playerProfile.osuUserId;
                const discordId = playerProfile.discordId;

                await addMatchLimitation(discordId);

                const lastCheck = playerProfile.lastNameCheck || 0;
                const shouldCheckName = Date.now() - lastCheck > 24 * 60 * 60 * 1000;

                let currentIngameName = dbName;

                if (shouldCheckName) {
                    try {
                        // Prefer Legacy v1 — much faster & higher rate limits
                        let osuUserData = legacyUserCache.get(userId);
                        if (!osuUserData) {
                            osuUserData = await legacy.getUser(userId);
                            legacyUserCache.set(userId, osuUserData);
                        }

                        const apiName = osuUserData.username.replace(/ /g, "_");

                        if (apiName !== dbName) {
                            currentIngameName = apiNameName;
                            ...
                            // Update local variables if 1v1
                            if (gameMode === "1v1") {
                                if (i === 0) osuUser1 = apiName;
                                else osuUser2 = apiName;
                            }
                            playerArray[i] = apiName;
                        }
                    } catch (err) {
                        if (err.statusCode === 429) {
                            console.warn("Legacy API 429 during name check skipping for now");
                        } else {
                ...
                let playerElo = await getElo(currentIngameName, mode);
                if (playerElo === 0) {
                    const starting = await startingElo(userId);
                    await addElo(currentIngameName, starting, mode);
                    playerElo = starting;
                } else {
                    playerElo = await getElo(currentIngameName, mode);
                }
            ...
            if (eloAvg >= HIGH_ELO_MATCHES && Array.isArray(interaction)) {
            ...
            channelName = channelName.replace(/Sussy/g, "Fussy");
            channelName = channelName.replace(/sussy/g, "Fussy");
                    ...
                    case states.Ended:
                        ...
                        for (let i=0; i<playerArray.length; i++) {
                            ...
                            if (!tournament && mode <= 2 && isMatchmaking) {
                                const placementMatches = playerProfile.matchRecord?.[gameMode]?.placementMatches ?? 0;
                                if (placementMatches < 5) { 
                                    if (playerElo < 1800 && playerElo > 1200) {
                                        const playerTeam = i < mode ? 0 : 1;
                                        const matchScore = [match.score[playerTeam], match.score[playerTeam == 0 ? 1 : 0]];
                                        const placementElo = await getPlacementElo(i < mode ? teamAavg : teamBavg, i < mode ? teamBavg : teamAavg, matchScore);

                                        eloDiff = i < mode ? placementElo - teamAavg : placementElo - teamBavg;
                                        eloDiff = Math.round(eloDiff);
                                    }

                                    await osuUser.updateOne({ osuUserId: playerProfile.osuUserId }, {
                                        $set: {
                                            [`matchRecord.${gameMode}.placementMatches`]: placementMatches + 1
                                        }
                                    });
                                }

                                // Bonus ELO based on points won
                                if (eloDiff < 0 && placementMatches > 5) eloDiff += match.score[i < mode ? 0 : 1];

                                // Bonus ELO based on win streaks
                                let winStreak = playerProfile.matchRecord?.[gameMode]?.winStreak ?? 0;
                                if (wonMatch && placementMatches >= 5) {
                                    winStreak += 1;

                                    if (winStreak >= 3) {
                                        eloDiff += Math.min(winStreak, 5);
                                    }
                                } else if (!wonMatch) {
                                    winStreak = 0;
                                }
                                ...
                                await osuUser.updateOne({ osuUserId: playerProfile.osuUserId }, {
                                    $set: {
                                        ...
                                        [`matchRecord.${gameMode}.winStreak`]: winStreak
                                    }
                                });
                ...
                if (matchState != states.Joined && matchState != states.Ended) {
                    try {   
                        ...
                        if (!playersRemaining || playersRemaining.length === 0) {
                            absentTimer = setTimeout(handleAbsence, 60 * 1000 * 2);
                            return;
                        }

                        // Detect which team this player was on
                        const playerIndex = playerArray.findIndex(p => p == playerName);
                        ...
                        const teamStillHasPlayers = lobby.slots.some(s => {
                            ...
                            if (sName) {
                                let playerGameIndex = playerArray.findIndex(p => p == sName);

                                if (playerTeam === 0) {
                                    return playerGameIndex < mode && playerGameIndex > -1;
                                } else {
                                    return playerGameIndex >= mode;
                                }
                            }
                        });
                        ...
                        if (!teamStillHasPlayers) {
                            ...
                            await lobby.channel.sendMessage(`${leftTeamName} has left. 120 seconds until auto ff.`);
