...
module.exports = {
    async getPlayerRank(player, mode, elo, matches) {
        if (player) {
            ...
            let gamesPlayed = playerProfile.matchRecord[mode].placementMatches ?? 0;
    ...
    async getRankProgress(discordId, mode, diff) {
        ...
        const matches = profile.matchRecord[mode].placementMatches ?? 0;
        const winStreak = profile.matchRecord[mode].winStreak ?? 0;
        const winStreakText = winStreak >= 3 ? ` 🔥 ${winStreak} Streak` : '';
        ...
        if (!nextElo || currentRank === 'Quantum') {
            ...
            return `Current ${mode} Rank: ${currentRank} ${eloRankAsEmojis(currentRank)} ${bold(`${elo}`)} (${italic(`${diff}`)}) ${winStreakText}\nYou are #${userEloRank} in RomAI!`;
        }
        ...
        const results = [];
        results.push(`Current ${mode} Rank: ${currentRank} ${eloRankAsEmojis(currentRank)} ${bold(`${elo}`)} (${italic(`${diff}`)}) ${winStreakText}`);
