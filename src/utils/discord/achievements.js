        ...
        try {
            ...
            async function checkTotalMatches() {
                ...
                await evaluateAchievement({
                    ...
                    messageFn: tier => `This means you have played a total of ${tier.value} matches <3`,
                });
            }

            async function checkWinStreak() {
                const winStreakDuel = playerProfile.matchRecord["1v1"].winStreak || 0;
                const winStreakDuo = playerProfile.matchRecord["2v2"].winStreak || 0;
                const maxWinStreak = Math.max(winStreakDuel, winStreakDuo);

                await evaluateAchievement({
                    achievementName: "Win Streak",
                    currentValue: maxWinStreak,
                    compareFn: tier => parseInt(tier.value) <= maxWinStreak,
                    messageFn: tier => `Amazing! A win streak of ${tier.value} Keep it up!`,
                });
            }
