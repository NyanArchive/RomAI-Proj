    ...
    for (let i = 0; i < players.length; i++) {
        ...
        for (let j = i + 1; j < players.length; j++) {
            ...
            */
            ...
            const highEloRule = (player1.elo >= 2000 && player2.elo >= 1900) || (player2.elo >= 2000 && player1.elo >= 1900);
            //const lowEloRule = (player1.elo <= 1200 && player2.elo <= 1300) || (player2.elo <= 1200 && player1.elo <= 1300);

            // Check if the ELO difference is less than the rankLimit
            if ((eloRange <= rankLimit || highEloRule) && canBeMatched) {
...
async function matchDuosPlayers(players) {
    ...
    const rankLimit = 180;
    const soloEloLimit = 200;   // difference limit between solo teammates
    ...
    soloPlayers.sort((a, b) => a.elo - b.elo);
    ...
    while (soloPlayers.length >= 2) {
        ...
        for (let i = 0; i < soloPlayers.length; i++) {
            for (let j = i + 1; j < soloPlayers.length; j++) {
                ...
                if (diff <= soloEloLimit && diff < bestDiff) {
                    bestDiff = diff;
                    bestPair = [i, j];
                }
            }
    ...
    for (let i = 0; i < duos.length; i++) {
        ...
        if (p1.elo > 2000) duoAelo -= (p1.elo - 2000);
        if (p2.elo > 2000) duoAelo -= (p2.elo - 2000);
        ...
        for (let j = i + 1; j < duos.length; j++) {
            ...
            if (p3.elo > 2000) duoBelo -= (p3.elo - 2000);
            if (p4.elo > 2000) duoBelo -= (p4.elo - 2000);
