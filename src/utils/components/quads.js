...
module.exports = {
    async quadsRequest(interaction, client, discordUser, reqUsers, selectedPool, customBO, customELO) {
        try {
            ...
            const quadsEmbed = new EmbedBuilder()
                .setDescription(`${selectedPoolText} ${discordUser.username} Has invited${embedText} to a Quads match!\nTeam A: <@${discordUser.id}> <@${reqUsers[0].id}> <@${reqUsers[1].id}> <@${reqUsers[2].id}>\nTeam B: <@${reqUsers[3].id}> <@${reqUsers[4].id}> <@${reqUsers[5].id}> <@${reqUsers[6].id}>\nThe invite will expire ${dateConversion(Date.now() + 120000)}`)
                .addFields({
            ...
            collector.on('collect', async (inter) => {
                if (inter.customId == acceptId) {
                    ...
                    quadsEmbed.data.fields[indexId + 1].value = `✅ Ready`;
                    ...
                    if (filterIds.every(id => id == 'done')) {
                        ...
                        let teams = {
                            teamA: [osuUserProfile.osuUserName, reqUserProfiles[0].osuUserName, reqUserProfiles[1].osuUserName, reqUserProfiles[2].osuUserName],
                            teamB: [reqUserProfiles[3].osuUserName, reqUserProfiles[4].osuUserName, reqUserProfiles[5].osuUserName, reqUserProfiles[6].osuUserName]
                        };
            ...
            collector.on('end', async () => {
                ...
                await interaction.editReply({
                    content: `${failedText} failed to accept the Quads match.`,
                    embeds: [],
                    components: []
                });
            })
